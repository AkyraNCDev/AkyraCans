  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              conn.send2ButtonLoc(jid, action === 'add' ? wel : lea, text, action === 'add' ? 'INTRO OR KICK?' : 'Be Like : Kena Mental? Out Solusinya', '⋮☰ Menu', '#menu', 'Info Grup', '.infogc', false, {
                contextInfo: {
                  mentionedJid: [user]
                }
              })
      ...
      case 'demote':
        ...
        if (chat.detect) this.send2ButtonLoc(jid, banp, text, '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', 'MENU', '#menu', 'OWNER', '#owner', null, {
          contextInfo: {
...
`.trim(), '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', 'DISABLE DELETE', '.disable delete', {
      quoted: m.message,
      contextInfo: {
