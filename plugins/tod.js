...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send3Button(m.chat, 'Truth or Dare', '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', 'Truth', ',truth', 'Dare', ',dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`, m)
  }
  if (/^truth$/i.test(command)) {
    ...
    conn.send2Button(m.chat, json.message, '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', 'Truth', ',truth', 'Dare', ',dare', m)
