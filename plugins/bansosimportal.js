...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    global.DATABASE.data.users[m.sender].lastbansos6 = global.db.data.users[m.sender].lastbansos6 || 0
    let randomaku = `${Math.floor(Math.random() * 120)}`.trim()
    let randomkamu = `${Math.floor(Math.random() * 70)}`.trim() //hehe Biar Susah Menang :v
    let Aku = (randomaku * 1)
    ...
    let __timers = (new Date - global.db.data.users[m.sender].lastbansos6)
