...
const ftroli = {
	key : {
       ...
       message: {
                    orderMessage: {
                            itemCount : 9999999999999999999,
                            status: 1,
                            surface : 1,
                            message: 'Developer A&A', //Kasih namalu
                            orderTitle: 'Azhari-Botz',
...
let package = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json')))
    ...
    const ultah = new Date('September 12, 2022 23:59:59')
    const ultah2 = new Date('December 20, 2022 23:59:59')
    const sekarat = new Date().getTime() 
    const sekarat2 = new Date().getTime() 
    const Kurang = ultah - sekarat
    const Kurangg = ultah2 - sekarat2
    ...
    const oodetek = Math.floor( Kurangg % (1000 * 60) / 1000)
...
`
   ...
   conn.sendMessage(m.chat, text.trim(), 'extendedTextMessage', { detectLinks: true, thumbnail: ppnya, quoted: fvideo, contextInfo : { mentionedJid: conn.parseMention(text),
