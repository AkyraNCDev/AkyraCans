...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        await this.sendFile(m.chat, json.nowm, '', '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', m)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(url).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(url).then(async res => {
            ...
            await m.reply(wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '🎀𝚁𝚒𝚔𝚔𝚊 𝙱𝙾𝚃', m)
            }
        }).catch(_ => _)
    }
